    ...
    *Read-only* representation of an attribute.
    ...
    def evolve(self, **changes):
        ...
        if "alias" in changes and "alias_is_default" not in changes:
            ...
            _OBJ_SETATTR.__get__(new)("alias", _default_init_alias_for(new.name))
    ...
    def __setstate__(self, state):
        ...
        if len(state) < len(self.__slots__):
            ...
            alias_is_default = state_dict.get("alias") is None or state_dict.get(
                "alias"
            ) == _default_init_alias_for(state_dict["name"])

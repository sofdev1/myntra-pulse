...
class Git(metaclass=_GitMeta):
    ...
    * https://learn.microsoft.com/en-us/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessw
    ...
    @classmethod
    def polish_url(
        cls, url: str, is_cygwin: Literal[False] = ..., expand_vars: bool = ...
    ) -> str: ...

    @overload
    @classmethod
    def polish_url(
        cls, url: str, is_cygwin: Union[None, bool] = None, expand_vars: bool = True
    ) -> str: ...
            ...
            if option_token.startswith("-") and not option_token.startswith("--"):
                for option_char in option_token[1:]:
                    ...
                    if option_char not in clusterable_short_options:
                        break
            if not (
                option.startswith("--") or (options_are_kwargs and len(candidate) > 1)
            ):
                continue
    ...
    def _option_candidates(
        cls, args: Sequence[Any] = (), kwargs: Optional[Mapping[str, Any]] = None
    ) -> List[str]:
        """Collect possible option spellings before command-line transformation."""
        options = [
            option
            for option in cls._unpack_args([arg for arg in args if arg is not None])
            if option.startswith("-")
        ]
        if kwargs:
            ...
            for key, value in kwargs.items():
                ...
                if any(
                    value is True or (value is not False and value is not None)
                    for value in values
                ):
                    key = str(key)
                    if len(key) != 1:
        ...
        **kwargs: Any,
        ...
        if not allow_unsafe_options:
            ...
            Git.check_unsafe_options(
                ...
                unsafe_options=self.unsafe_git_ls_remote_options,
            )

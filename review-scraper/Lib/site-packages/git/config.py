...
class GitConfigParser(cp.RawConfigParser, metaclass=MetaParserBuilder):
    ...
    def read(self) -> None:  # type: ignore[override]
        ...
        files_to_read = [
            osp.abspath(path) if isinstance(path, (str, os.PathLike)) else path
    ...
    def _assure_config_name_safe(self, name: "cp._SectionName", label: str) -> None:
        if isinstance(name, str) and UNSAFE_CONFIG_CHARS_RE.search(name):
            raise ValueError(
                "Git config %s names must not contain CR, LF, or NUL" % label
            )
        if (
            label == "option"
            and isinstance(name, str)
            and not VALID_CONFIG_OPTION_NAME_RE.fullmatch(name)
        ):
            raise ValueError(
                "Git config option names may contain only letters, digits, '-', '_', or '.'"
            )
            ...
            for index, char in enumerate(name):
                if escaped:
                    ...
                    if not in_quotes and (index == 0 or name[index - 1] not in " \t"):
                        raise ValueError(
                            "Git config quoted subsection names must begin after whitespace"
                        )
                    in_quotes = not in_quotes
                elif char == "]" and not in_quotes:
                    raise ValueError(
                        "Git config section names must not contain an unquoted closing bracket"
                    )
            if in_quotes:
                raise ValueError(
                    "Git config section names must not contain an unterminated quote"
                )

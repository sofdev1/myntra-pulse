...
from git.util import (IterableList, RemoteProgress, _to_relative_path,
...
class Submodule(IndexObject, TraversableIterableObj):
    ...
    def _clone_repo(
        ...
        **kwargs: Any,
        ...
        if url.startswith("../"):
            remote_name = cast(
                "RemoteReference", repo.active_branch.tracking_branch()
            ).remote_name
                ...
                if not dry_run and osp.isdir(module_abspath):
                    try:
                        ...
                        self._write_git_file_and_module_config(
                            checkout_module_abspath, module_abspath
                        )
                        mrepo = git.Repo(checkout_module_abspath)
                        mrepo.head.reset(
                            mrepo.head.commit, index=True, working_tree=True
                        )
                        fetch_remotes(mrepo)

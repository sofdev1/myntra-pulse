...
from git.util import (Actor, LazyMixin, LockedFD, _is_path_rooted,
                      _to_relative_path, file_contents_ro, join_path_native,
...
class IndexFile(LazyMixin, git_diff.Diffable, Serializable):
        ...
        if self.repo.bare:
            ...
            if drive or _is_path_rooted(path):
                raise InvalidGitRepositoryError(
                    "paths with a drive or root require a non-bare repository"
                )
            return path
    ...
    def _entries_for_paths(
        ...
        if path_rewriter:
            ...
            if working_tree_dir is None:
                raise InvalidGitRepositoryError(
                    "Cannot rewrite paths without a working tree"
                )
    ...
    def reset(
        ...
        **kwargs: Any,
        ...
        new_inst = type(self).from_tree(
            self.repo, commit, allow_unsafe_options=allow_unsafe_options
        )
        if not paths:
    ...
    def diff(
        ...
        **kwargs: Any,
        ...
        if other == git_diff.NULL_TREE or other == git_diff.NULL_TREE_SHA:
            ...
            diff_method = (
                git_diff.Diff._index_from_patch_format
                if create_patch
                else git_diff.Diff._index_from_raw_format
            )

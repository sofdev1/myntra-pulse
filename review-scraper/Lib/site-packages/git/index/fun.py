...
from .typ import CE_EXTENDED, CE_NAMEMASK, CE_STAGESHIFT, BaseIndexEntry, IndexEntry
...
def _commit_hook_path(name: str, index: "IndexFile") -> str:
    ...
    return osp.abspath(
        osp.join(index.repo.working_dir, osp.expanduser(hooks_dir), name)
    )
...
def _git_for_windows_root() -> Union[str, None]:
    ...
    if osp.dirname(git_executable):
        ...
        names = (
            (git_executable,)
            if _has_file_extension(git_executable)
            else (git_executable, f"{git_executable}.exe")
        )
        for name in names:
...
def run_commit_hook(name: str, index: "IndexFile", *args: str) -> None:
    ...
    try:
        if sys.platform == "win32" and not _has_file_extension(hp):
            ...
            bash_executable = (
                _git_for_windows_bash() or _which_from_path("bash.exe") or "bash.exe"
            )
...
def read_header(stream: IO[bytes]) -> Tuple[int, int]:
    ...
    assert version in (1, 2, 3), (
        "Unsupported git index version %i, only 1, 2, and 3 are supported" % version
    )

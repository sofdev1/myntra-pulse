...
def _common_reflog_path(repo: "Repo", ref: SymbolicReference) -> Optional[str]:
    ...
    return SymbolicReference._get_validated_path(
        osp.join(repo.common_dir, "logs"), ref.path
    )
...
def _tracking_branch_object(
    repo: "Repo", ref: Optional[SymbolicReference]
) -> AnyGitObject:
...
def _apply_reflog(
    repo: "Repo", ref: Optional[SymbolicReference], content: str
) -> AnyGitObject:
...
def _find_commit_by_message(
    ...
    pattern, negated = _parse_search(
        _unescape_braced_regex(pattern) if braced else pattern
    )
    try:
...
def _index_lookup(repo: "Repo", spec: str) -> AnyGitObject:
    ...
    try:
        ...
        raise BadName(
            "Path %r did not exist in the index at stage %i" % (path, stage)
        ) from e
...
def rev_parse(repo: "Repo", rev: str) -> AnyGitObject:
    ...
    if first_token == 0:
        if rev[0] != "@":
            raise ValueError(
                "Revision specifier must start with an object name: %s" % rev
            )
        ...
        if rev[first_token] == "@":
            ref = cast(
                "Reference", name_to_object(repo, rev[:first_token], return_ref=True)
            )
            obj = ref.commit
        else:
    ...
    while start < lr:
        ...
        if token == "@":
            ...
            obj = _apply_reflog(
                repo,
                ref if first_token != 0 and start == first_token else None,
                rev[start + 2 : end],
            )
            ref = None
            start = end + 1

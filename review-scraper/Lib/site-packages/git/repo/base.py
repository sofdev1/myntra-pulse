    ...
    * :attr:`git_dir` is the ``.git`` repository directory, which is always set.
            ...
            if (
                ...
                if "GIT_WORK_TREE" in os.environ:
                    ...
                    if not osp.isabs(worktree_gitfile):
                        worktree_gitfile = osp.normpath(
                            osp.join(git_dir, worktree_gitfile)
                        )
    ...
    def blame(
        ...
        **kwargs: Any,
        ...
        """
        if incremental:
            return self.blame_incremental(
                rev, file, allow_unsafe_options=allow_unsafe_options, **kwargs
            )
        ...
        data: bytes = self.git.blame(
            rev, *rev_opts_list, "--", file, p=True, stdout_as_string=False, **kwargs
        )
    ...
    def _clone(
        ...
        **kwargs: Any,
        ...
        if not allow_unsafe_options and multi:
            Git.check_unsafe_options(
                options=multi, unsafe_options=cls.unsafe_git_clone_options
            )
        ...
        if repo.remotes:
            with repo.remotes[0].config_writer as writer:
                writer.set_value(
                    "url", Git.polish_url(repo.remotes[0].url, expand_vars=False)
                )

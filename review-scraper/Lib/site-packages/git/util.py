...
def _to_relative_path(root: PathLike, path: PathLike) -> str:
    ...
    * ``C:\other\file`` -> :exc:`ValueError`
    ...
    try:
        ...
        raise ValueError(
            "Path %r is not in repository at %r" % (path_str, root_abs)
        ) from e
...
def _cygexpath(drive: Optional[str], path: str, expand_vars: bool = True) -> str:
    if osp.isabs(path) and not drive:
        ...
        p = path and osp.normpath(
            osp.expandvars(osp.expanduser(path)) if expand_vars else path
        )
        if osp.isabs(p):
            if drive:
...
def finalize_process(
    proc: Union["subprocess.Popen[Any]", "Git.AutoInterrupt"], **kwargs: Any
) -> None:
...
class Actor:
    ...
    def _main_actor(
        ...
        env_email: str,
        config_reader: Union[
            None, "GitConfigParser", "SectionConstraint[GitConfigParser]"
        ] = None,
    ) -> "Actor":
        actor = Actor("", "")
...
class IterableClassWatcher(type):
    ...
    def __init__(
        cls, name: str, bases: Tuple[type, ...], clsdict: Dict[str, Any]
    ) -> None:

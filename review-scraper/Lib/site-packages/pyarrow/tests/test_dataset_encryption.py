...
def create_kms_connection_config(keys=KEYS):
    return pe.KmsConnectionConfig(
        custom_kms_conf={
            key_name: key.decode("UTF-8") for key_name, key in keys.items()
        }
    )
...
def do_test_dataset_encryption_decryption(table, extra_column_path=None):
    ...
    plaintext_column_names = [
        ...
        and (
            extra_column_path is None
            or not extra_column_path.startswith(f"{column_name}.")
        )
    ]
    ...
    column_keys_only = {
        key_name: key for key_name, key in keys.items() if key_name != FOOTER_KEY_NAME
    }
...
def test_dataset_encryption_with_selected_column_statistics():
    ...
    pformat = pa.dataset.ParquetFileFormat()
    write_options = pformat.make_write_options(
        encryption_config=parquet_encryption_cfg, write_statistics=["year", "n_legs"]
    )

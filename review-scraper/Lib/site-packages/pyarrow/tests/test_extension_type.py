...
def test_array_from_extension_scalars():
    ...
    custom_cases = [
        (IntegerType(), [100, 200]),
        (AnnotatedType(pa.timestamp("us"), "ts"), [datetime.datetime(2023, 1, 1)]),
    ...
    uuid_type = pa.uuid()
    scalars = [
        pa.scalar(b"0123456789abcdef", type=uuid_type),
...
def test_tensor_class_methods(np_type_str):
    ...
    result = pa.ExtensionArray.from_storage(tensor_type, storage)
    expected = as_strided(
        flat_arr, shape=(1, 3, 2, 2), strides=(bw * 12, bw, bw * 3, bw * 6)
    )
    np.testing.assert_array_equal(result.to_numpy_ndarray(), expected)
...
def test_tensor_array_from_list_of_0d_arrays():
    ...
    result = pa.array(
        [np.array(1, dtype=np.int64), np.array(2, dtype=np.int64)], type=tensor_type
    )
    assert result.type == tensor_type

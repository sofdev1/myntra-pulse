...
class EchoTableStreamFlightServer(EchoFlightServer):
    ...
    def do_get(self, context, ticket):
        return flight.GeneratorStream(self.last_message.schema, [self.last_message])
...
class EchoRecordBatchReaderStreamFlightServer(EchoFlightServer):
    ...
    def do_get(self, context, ticket):
        return flight.GeneratorStream(
            self.last_message.schema, [self.last_message.to_reader()]
        )
...
def test_flight_generator_stream_of_batches_with_dict():
    ...
    with EchoRecordBatchReaderStreamFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        writer, _ = client.do_put(flight.FlightDescriptor.for_path("test"), data.schema)
        writer.write_table(data)
...
def test_flight_generator_stream_of_table():
    ...
    with EchoTableStreamFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        writer, _ = client.do_put(flight.FlightDescriptor.for_path("test"), data.schema)
        writer.write_table(data)
...
def test_flight_dictionary_deltas_do_exchange():
    ...
    with DeltaFlightServer() as server, FlightClient(
        ("localhost", server.port)
    ) as client:
        expected_table = simple_dicts_table()
        for command in ["dict_deltas", "dict_replacement"]:
            ...
            writer, reader = client.do_exchange(
                descriptor,
                options=flight.FlightCallOptions(
                    write_options=pa.ipc.IpcWriteOptions(emit_dictionary_deltas=True)
                ),
            )
...
@pytest.mark.parametrize("call_options_args", ["default", "all"], indirect=True)

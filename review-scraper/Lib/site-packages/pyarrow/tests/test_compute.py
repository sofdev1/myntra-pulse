...
def test_option_class_equality(request):
    options = [
        ...
        pc.RandomOptions(),
        pc.RankOptions(
            sort_keys="ascending", null_placement="at_end", tiebreaker="max"
        ),
        pc.RankQuantileOptions(sort_keys="ascending", null_placement="at_end"),
...
def _compare_strftime_strings_on_windows(result, expected):
    ...
    assert all_end_with_tz, (
        "All timezone values should be GMT offset format, "
...
def test_assume_timezone():
    ...
    with pytest.raises(
        ValueError, match="not found in timezone database|" "unable to locate time_zone"
    ):
...
def test_count_run_end_encoded_nulls():
    arr = pc.run_end_encode(pa.array([1, 1, None, None, None, 2, 2, 2, None, 3]))
...
def test_select_k_array():
    def validate_select_k(
        select_k_indices, arr, order, null_placement="at_end", stable_sort=False
    ):
    ...
    result = pc.select_k_unstable(
        arr,
        options=pc.SelectKOptions(k=2, sort_keys=[("dummy", "descending", "at_end")]),
    )
    validate_select_k(result, arr, "descending")

    result = pc.select_k_unstable(
        arr,
        options=pc.SelectKOptions(k=2, sort_keys=[("dummy", "ascending", "at_end")]),
    )
    validate_select_k(result, arr, "ascending")

    # Position options
    assert (
        pc.select_k_unstable(arr, 2, sort_keys=[("dummy", "ascending", "at_end")])
        == result
    )
...
def test_select_k_table():
    ...
    for k in [0, 2, 4]:
        ...
        validate_select_k(
            result,
            table,
            sort_keys=[("a", "ascending", "at_end"), ("b", "ascending", "at_end")],
        )

        result = pc.top_k_unstable(
            table, k=k, sort_keys=["a"], null_placements=["at_end"]
        )
        ...
        validate_select_k(
            result,
            table,
            sort_keys=[("a", "ascending", "at_end"), ("b", "ascending", "at_start")],
        )
...
def test_sort_indices_table():
    ...
    result = pc.sort_indices(
        table,
        sort_keys=[("a", "descending", "at_start"), ("b", "ascending", "at_start")],
    )
    assert result.to_pylist() == [2, 1, 0, 3]
    # Positional `sort_keys`
    result = pc.sort_indices(
...
def test_rank_options():
    ...
    result = pc.rank(
        arr, options=pc.RankOptions(sort_keys=[("b", "ascending", "at_end")])
    )
    assert result.equals(expected)

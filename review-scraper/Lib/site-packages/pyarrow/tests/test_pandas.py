...
class TestConvertMetadata:
    ...
    def test_multiindex_rangeindex(self):
        ...
        multiindex = pd.MultiIndex.from_arrays([pd.RangeIndex(0, 2), pd.Index([1, 2])])
    ...
    def test_index_level_name_bump_multiindex(self):
        ...
        df = pd.DataFrame(
            ...
            index=pd.MultiIndex.from_arrays([[10, 20], [100, 200]], names=[None, None]),
        )
        _check_pandas_roundtrip(df, preserve_index=True)
    ...
    def test_mismatch_metadata_schema(self):
        ...
        if Version(pd.__version__) >= Version("2.0.0"):
            ...
            ext_kwargs = {"unit": "ns"}
        df = pd.DataFrame(
            {"datetime": pd.date_range("2020-01-01", periods=3, **ext_kwargs)}
        )
...
class TestConvertMisc:
    ...
    def test_empty_arrays(self):
        for dtype_str, pa_type in self.type_pairs:
            if Version(pd.__version__) >= Version("3.0.0") and pa_type == pa.string():
...
def test_dictionary_with_pandas():
    ...
    for index_type in [
        ...
        if index_type == "uint64":
            ...
            with pytest.raises(TypeError, match="UInt64 dictionary indices"):
                d1.to_pandas()
            continue
...
def test_array_to_pandas_string_dtype(string_type, data):
    ...
    assert result.dtype == pd.CategoricalDtype(categories=[], ordered=False)

...
def test_sparse_union_array_slice_with_nulls():
    ...
    ints_with_offset = pa.array([999, None, 20, 30, 40, 50], type=pa.int64())[1:]
...
def test_create_date32_and_date64_arrays_with_mask():
    ...
    arr_date32 = pa.array(
        [0, 0, 1, 2], mask=[False, False, True, False], type=pa.date32()
    )
    ...
    arr_date64 = pa.array(
        [0, 86400000, 172800000, 259200000],
    ...
    arr_all_null = pa.array(
        [0, 1, 2, 3], mask=[True, True, True, True], type=pa.date32()
    )
...
def test_array_from_different_numpy_datetime_units_raises():
    ...
    with pytest.raises(
        pa.ArrowInvalid, match="Cannot mix NumPy datetime64 units s and ms"
    ):
        pa.array(data)
...
def test_array_from_unsupported_numpy_datetime_unit_names(unit):
    ...
    with pytest.raises(
        pa.ArrowInvalid, match=f"Cannot mix NumPy datetime64 units s and {unit}"
    ):
        pa.array(data)
...
def test_array_from_timestamp_with_generic_unit():
    ...
    with pytest.raises(pa.ArrowInvalid, match="Cannot mix NumPy datetime64 units"):
        pa.array([n, x, y])
...
def test_fixed_size_list_from_multidim_ndarray():
    ...
    arr = pa.array(
        [np.array([[1, 2], [3, 4]], dtype=np.int64)], type=pa.list_(pa.int64(), 4)
    )
    assert arr.to_pylist() == [[1, 2, 3, 4]]

    with pytest.raises(pa.lib.ArrowInvalid):
        pa.array(
            [np.array([[1, 2], [3, 4]], dtype=np.int64)], type=pa.list_(pa.int64(), 3)
        )

...
def test_parquet_read_write_table_raw_fd(tempdir):
    ...
    fd = os.open(path, os.O_CREAT | os.O_WRONLY | os.O_TRUNC | binary_flag, 0o666)
...
def test_bloom_filter_options():
    ...
    _check_roundtrip(
        table, expected=table, bloom_filter_options={"a": {"ndv": 100, "fpp": 0.05}}
    )
    ...
    _check_roundtrip(table, expected=table, bloom_filter_options={"a": {"fpp": 0.05}})

    # bloom filter for one column with default fpp
    _check_roundtrip(table, expected=table, bloom_filter_options={"a": {"ndv": 100}})

    # bloom filter for one column with default ndv and fpp
    _check_roundtrip(table, expected=table, bloom_filter_options={"a": {}})
    _check_roundtrip(table, expected=table, bloom_filter_options={"a": True})

    # should remain disabled
    _check_roundtrip(table, expected=table, bloom_filter_options={"a": False})
    ...
    with pytest.raises(TypeError, match=expect):
        _write_table(
            table, buf, bloom_filter_options={"a": {"ndv": "100", "fpp": 0.05}}
        )
    ...
    with pytest.raises(ValueError, match=expect):
        _write_table(table, buf, bloom_filter_options={"a": {"ndv": -10}})
    ...
    with pytest.raises(ValueError, match=re.escape(expect)):
        _write_table(table, buf, bloom_filter_options={"a": {"fpp": 2.0}})
    expect = "'bloom_filter_options:fpp' for column 'a' must be in (0.0, 1.0), got -0.5"
    with pytest.raises(ValueError, match=re.escape(expect)):
        _write_table(table, buf, bloom_filter_options={"a": {"fpp": -0.5}})

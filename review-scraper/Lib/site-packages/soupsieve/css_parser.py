...
CSS_REQUIRED = CSSPattern(
    "html|*:is(input, textarea, select)[required]", FLG_PSEUDO | FLG_HTML
)
# CSS pattern for `:optional`
CSS_OPTIONAL = CSSPattern(
    "html|*:is(input, textarea, select):not([required])", FLG_PSEUDO | FLG_HTML
)
...
class CSSParser:
    ...
    def check_count(self) -> None:
        ...
        if self.count > SELECTOR_LIMIT:
            raise ValueError(
                f"Selector exceeds pseudo-class nesting limit of {SELECTOR_LIMIT}"
            )
    ...
    def parse_selectors(
        ...
        return ct.SelectorList(
            [s.freeze() for s in selectors], is_not, is_html, self.count - count
        )

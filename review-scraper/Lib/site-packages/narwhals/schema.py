...
from narwhals.dependencies import (get_cudf, is_cudf_dtype,
                                   is_pandas_like_dtype, is_polars_data_type,
                                   is_polars_schema, is_pyarrow_data_type,
                                   is_pyarrow_schema)
...
if TYPE_CHECKING:
    ...
    from narwhals.typing import (DTypeBackend, IntoArrowSchema, IntoDType,
                                 IntoPandasSchema, IntoPolarsSchema)
...
class Schema(OrderedDict[str, "DType"]):
    ...
    def __init__(
        ...
        if schema is None:
            ...
            super().__init__(
                ...
                for name, dtype in (
                    schema.items() if _supports_items(schema) else schema
                )
            )
    ...
    def _from_native_mapping(
        cls,
        native: (
            Mapping[str, pa.DataType] | Mapping[str, pl.DataType] | IntoPandasSchema
        ),
        /,
    ) -> Self:

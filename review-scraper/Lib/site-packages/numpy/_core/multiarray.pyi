...
from typing import (Never, Protocol, Self, SupportsIndex, final, overload,
                    type_check_only)
...
from numpy import (_CastingKind, _CopyMode, _ModeKind, _NDIterFlagsKind,
                   ...
                   broadcast, complexfloating, correlate, count_nonzero,
                   datetime64, dtype)
from numpy import einsum as c_einsum
from numpy import (float64, floating, from_dlpack, int_, interp, intp, matmul,
                   ndarray, signedinteger, str_, timedelta64, ufunc, uint8,
...
type _ArrayJustND[ScalarT: np.generic] = ndarray[
    tuple[Never, Never, Never, Never], dtype[ScalarT]
]
...
type _ToArray3D[ScalarT: np.generic] = _Array3D[ScalarT] | Sequence[
...
def scalar[DTypeT: np.dtype](
    dtype: DTypeT, object: bytes | object = ...
...
def inner[ScalarT: _InnerScalar | np.object_](
    a: _ArrayJustND[ScalarT], b: _ArrayLike[ScalarT], /
) -> NDArray[ScalarT] | Any: ...
@overload  # (Nd T, ?d T) -> 0d|Nd T  (workaround)
def inner[ScalarT: _InnerScalar | np.object_](
    a: _ArrayLike[ScalarT], b: _ArrayJustND[ScalarT], /
) -> NDArray[ScalarT] | Any: ...
...
def dot(
    a: _ArrayJustND[_DotScalar | np.object_],
    b: _ArrayLike[_DotScalar | np.object_],
    out: None = None,
) -> Any: ...
@overload  # (Nd _, ?d _) -> 0d|Nd _  (workaround)
def dot(
    a: _ArrayLike[_DotScalar | np.object_],
    b: _ArrayJustND[_DotScalar | np.object_],
    out: None = None,
) -> Any: ...
@overload  # (1d T, 1d T) -> 0d T
def dot[ScalarT: _DotScalar](
    a: _ToArray1D[ScalarT], b: _ToArray1D[ScalarT], out: None = None
) -> ScalarT: ...
@overload  # (1d object_, 1d _) -> 0d object
def dot(
    a: _Array1D[np.object_],
    b: _Array1D[np.object_] | _ToArray1D[_DotScalar],
    out: None = None,
) -> Any: ...
@overload  # (1d _, 1d object_) -> 0d object
def dot(
    a: _ToArray1D[_DotScalar], b: _Array1D[np.object_], out: None = None
) -> Any: ...
@overload  # (1d bool, 1d bool) -> bool_
...
def where(
    condition: _ArrayJustND[Any], x: None = None, y: None = None, /
) -> tuple[_Array1D[np.intp], ...]: ...
@overload  # (1d)
def where(
    condition: _ToArray1D[Any], x: None = None, y: None = None, /
) -> tuple[_Array1D[np.intp]]: ...
@overload  # (2d)
def where(
    condition: _ToArray2D[Any], x: None = None, y: None = None, /
) -> tuple[_Array1D[np.intp], _Array1D[np.intp]]: ...
@overload  # (3d)
def where(
...
def where(
    condition: _ArrayLike[Any], x: None = None, y: None = None, /
) -> tuple[_Array1D[np.intp], ...]: ...
@overload
def where(
...
class busdaycalendar:
    ...
    def __init__(
        ...
        /,
        weekmask: (
            str | Sequence[_IntLike_co] | _SupportsArray[NDArray[np.bool | np.integer]]
        ) = "1111100",
        holidays: (
            Sequence[dt.date | np.datetime64[dt.date]]
            | _SupportsArray[NDArray[np.datetime64[dt.date]]]
            | None
        ) = None,
    ) -> None: ...
    @property
...
class flatiter(Generic[_ArrayT_co]):
    ...
    def __next__[
        ScalarT: np.bool | np.number | np.flexible | np.datetime64 | np.timedelta64
    ](
    ...
    def __getitem__[ScalarT: np.generic](
        self: flatiter[NDArray[ScalarT]], key: int | np.integer, /
    ) -> ScalarT: ...

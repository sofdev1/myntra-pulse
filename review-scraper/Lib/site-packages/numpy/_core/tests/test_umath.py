...
from numpy.testing import (HAS_REFCOUNT, IS_MUSL, IS_WASM, _gen_alignment_data,
...
class TestPower:
    ...
    def test_power_complex(self):
        ...
        assert_almost_equal(
            x ** (0.5),
            [ncu.sqrt(1 + 2j), ncu.sqrt(2 + 3j), ncu.sqrt(3 + 4j)],
        )
        norm = 1.0 / ((x**14)[0])
        assert_almost_equal(
            x**14 * norm,
...
class TestBitwiseUFuncs:
    ...
    @pytest.mark.parametrize(
        "input_dtype_obj, bitsize", list(zip(bitwise_types, bitwise_bits))
    )
...
@pytest.mark.xfail(
    condition=LONG_DOUBLE_IS_IBM_DOUBLE_DOUBLE, reason="IBM double double"
)
def test_nextafterl():
...
def test_nextafter_signed_zero(sctype):
    ...
    assert _equal_signed_zero(
        np.nextafter(pos_zero, neg_zero), neg_zero
    ), f"nextafter(+0.0, -0.0) != -0.0 for {sctype.__name__}"
    assert _equal_signed_zero(
        np.nextafter(neg_zero, pos_zero), pos_zero
    ), f"nextafter(-0.0, +0.0) != +0.0 for {sctype.__name__}"

    assert _equal_signed_zero(
        np.nextafter(pos_zero, pos_zero), pos_zero
    ), f"nextafter(+0.0, +0.0) != +0.0 for {sctype.__name__}"
    assert _equal_signed_zero(
        np.nextafter(neg_zero, neg_zero), neg_zero
    ), f"nextafter(-0.0, -0.0) != -0.0 for {sctype.__name__}"
...
def test_abs_nan_signbit_array():
    ...
    assert_array_equal(
        result, [False, False], "abs of NaN array should have all positive signs"
    )

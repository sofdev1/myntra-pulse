...
class TestDateTime:
    ...
    def test_datetime_scalar_construction(self):
        ...
        with pytest.warns(
            DeprecationWarning, match=self.generic_unit_deprecation_message
        ):
        ...
        assert_equal(
            repr(np.datetime64("NaT", "D").astype(np.dtype("datetime64[ns]"))),
    ...
    def test_timedelta64_unary(self):
        for tda, tdb, tdzero in [
            ...
            # sign ufunc
            assert_equal(np.sign(tda), np.ones_like(tda, dtype=np.float64), strict=True)
            assert_equal(
                np.sign(tdb), -np.ones_like(tdb, dtype=np.float64), strict=True
            )
            assert_equal(
                np.sign(tdzero), np.zeros_like(tdzero, dtype=np.float64), strict=True
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # div with NaT
            (np.timedelta64("NaT", "us"), np.timedelta64(50, "us")),
            # special case for int64 min
            # in integer floor division
            (np.timedelta64(np.iinfo(np.int64).min, "s"), np.timedelta64(-1, "s")),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # handle 1D arrays
            (np.array([1, 2, 3], dtype="m8[ns]"), np.array([2], dtype="m8[ns]")),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # special case for int64 min
            # in integer floor division
            (np.timedelta64(np.iinfo(np.int64).min, "ns"), np.timedelta64(-1, "ns")),
        ],
    )
    ...
    def test_datetime_busday_offset(self):
        ...
        assert_equal(
            np.busday_offset(np.datetime64("NaT", "D"), 1, roll="nat"),
            np.datetime64("NaT", "D"),
        )
        assert_equal(
    ...
    @pytest.mark.parametrize("atol", [np.timedelta64(1, "s"), np.timedelta64(1, "ms")])

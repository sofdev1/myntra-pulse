...
from numpy.testing import (HAS_REFCOUNT, IS_64BIT, assert_, assert_array_equal,
...
class TestPickling:
    ...
    def test_setstate_invalid_tuple_size(self, dt):
        ...
        for size in [1, 2, 3, 4]:
            with pytest.raises(ValueError, match="Invalid state while unpickling"):
...
class TestDTypeClasses:
    @pytest.mark.parametrize("dtype", list(np.typecodes["All"]) + [rational, rational2])
...
class TestDTypeSignatures:
    ...
    @pytest.mark.parametrize(
        "typename", [n for n in np.dtypes.__all__ if n != "register_dlpack_dtype"]
    )

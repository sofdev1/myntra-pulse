...
def test_trailing_null_is_not_padding():
    ...
    assert_array_equal(np.strings.endswith(arr, nul), [True, True, True, False])
    assert_array_equal(
        np.strings.slice(arr, -1, None),
        np.array(["\0", "\0", "\0", "c"], dtype=StringDType()),
    )


def test_trailing_null_is_not_stripped_as_whitespace():
    arr = np.array(["x\0", "\0 ", " \0", "x\0 \t"], dtype=StringDType())
    ...
    assert_array_equal(
        np.strings.strip(arr), np.array(["x\0", "\0", "\0", "x\0"], dtype=StringDType())
    )
...
def test_indexing_ops_distinct_allocators():
    ...
    assert_array_equal(np.insert(a, 3, b[:4]), np.insert(a_obj, 3, b_obj[:4]))
...
def test_setops_distinct_allocators():
    ...
    assert_array_equal(np.isin(a, b, invert=True), np.isin(au, bu, invert=True))
...
def test_lexsort_distinct_allocators():
    ...
    sec = np.array([f"{'s' * 20}{(7 * i) % n:03d}" for i in range(n)], dtype="T")
...
def test_strings_ufuncs_distinct_allocators():
    ...
    expected = np.add(np.array(a_list, dtype=object), np.array(b_list, dtype=object))
    ...
    needles_list = [a_list[i][:16] if i % 3 else "Z" * 16 for i in range(n)]

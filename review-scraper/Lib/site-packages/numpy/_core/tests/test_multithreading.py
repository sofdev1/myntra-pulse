...
@pytest.mark.parametrize(
    "rat_cls", [_rational_tests.rational, _rational_tests.rational2]
)
...
def assert_no_deadlock(workload, *, args=(), helpers=(), timeout=30, reason="deadlock"):
...
def threaded_deadlock_reproducer(operation, nworkers, niters, stall):
    ...
    workers = [
        threading.Thread(target=worker, args=(i,), daemon=True) for i in range(nworkers)
    ]
    for t in workers:
        t.start()
...
@pytest.mark.parametrize(
    "case_",
    ["two-allocator", "three-allocator", "four-allocator", "duplicate-allocator"],
)

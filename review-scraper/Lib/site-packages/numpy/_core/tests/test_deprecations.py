...
class TestDeprecatedViewDtypePropertySetter(_DeprecationTestCase):
    ...
    def test_view_dtype_property_setter_recarray_subclass(self):
        ...
        arr = np.rec.fromarrays([np.arange(3, dtype=np.int32)], names="x").view(
            MyRecArray
        )
...
class TestDeprecatedGenericTimedelta(_DeprecationTestCase):
    ...
    @pytest.mark.parametrize("value", [3, 10, "NaT"])
    ...
    @pytest.mark.parametrize("value", [np.timedelta64(3, "s"), np.timedelta64(10, "D")])
    @pytest.mark.parametrize("generic_value", [5, 2])

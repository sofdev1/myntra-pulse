...
from numpy._typing import (DTypeLike, NDArray, _AnyShape, _ArrayLike,
                           _BoolCodes, _DTypeLike, _Float32Codes,
                           _Float64Codes, _Int8Codes, _Int16Codes, _Int32Codes,
                           _Int64Codes, _IntCCodes, _IntPCodes, _LongCodes,
                           _LongDoubleCodes, _LongLongCodes, _ShapeLike,
                           _UInt8Codes, _UInt16Codes, _UInt32Codes,
                           _UInt64Codes, _UIntCCodes, _UIntPCodes, _ULongCodes,
                           _ULongLongCodes, _VoidDTypeLike)
...
class _ndptr[OptionalDTypeT: np.dtype | None](ct.c_void_p):
    ...
    @classmethod
    def from_param(
        cls: type[_ndptr[None]], obj: np.ndarray
    ) -> _ctypes[Any]: ...  # pyrefly: ignore[bad-override]
    @overload
    @classmethod
    def from_param[DTypeT: np.dtype](
        cls: type[_ndptr[DTypeT]], obj: np.ndarray[Any, DTypeT]
    ) -> _ctypes[Any]: ...  # pyright: ignore[reportIncompatibleMethodOverride]
...
@overload  # bool
def as_ctypes_type(
    dtype: _BoolCodes | _DTypeLike[np.bool] | type[ct.c_bool],
) -> type[ct.c_bool]: ...
@overload  # int8
def as_ctypes_type(
    dtype: _Int8Codes | _DTypeLike[np.int8] | type[ct.c_int8],
) -> type[ct.c_int8]: ...
@overload  # int16
def as_ctypes_type(
    dtype: _Int16Codes | _DTypeLike[np.int16] | type[ct.c_int16],
) -> type[ct.c_int16]: ...
@overload  # int32
def as_ctypes_type(
    dtype: _Int32Codes | _DTypeLike[np.int32] | type[ct.c_int32],
) -> type[ct.c_int32]: ...
@overload  # int64
def as_ctypes_type(
    dtype: _Int64Codes | _DTypeLike[np.int64] | type[ct.c_int64],
) -> type[ct.c_int64]: ...
@overload  # intc
...
@overload  # intp
def as_ctypes_type(
    dtype: _IntPCodes | type[ct.c_ssize_t] | type[int],
) -> type[ct.c_ssize_t]: ...
@overload  # uint8
def as_ctypes_type(
    dtype: _UInt8Codes | _DTypeLike[np.uint8] | type[ct.c_uint8],
) -> type[ct.c_uint8]: ...
@overload  # uint16
def as_ctypes_type(
    dtype: _UInt16Codes | _DTypeLike[np.uint16] | type[ct.c_uint16],
) -> type[ct.c_uint16]: ...
@overload  # uint32
def as_ctypes_type(
    dtype: _UInt32Codes | _DTypeLike[np.uint32] | type[ct.c_uint32],
) -> type[ct.c_uint32]: ...
@overload  # uint64
def as_ctypes_type(
    dtype: _UInt64Codes | _DTypeLike[np.uint64] | type[ct.c_uint64],
) -> type[ct.c_uint64]: ...
@overload  # uintc
...
@overload  # float32
def as_ctypes_type(
    dtype: _Float32Codes | _DTypeLike[np.float32] | type[ct.c_float],
) -> type[ct.c_float]: ...
@overload  # float64
def as_ctypes_type(
    dtype: _Float64Codes | _DTypeLike[np.float64] | type[float | ct.c_double],
) -> type[ct.c_double]: ...
@overload  # longdouble
def as_ctypes_type(
    dtype: _LongDoubleCodes | _DTypeLike[np.longdouble] | type[ct.c_longdouble],
) -> type[ct.c_longdouble]: ...
@overload  # void
def as_ctypes_type(
    dtype: _VoidDTypeLike,
) -> type[Any]: ...  # `ct.Union` or `ct.Structure`
@overload  # fallback
...
@overload
def as_array[ScalarT: np.generic](
    obj: _ArrayLike[ScalarT], shape: _ShapeLike | None = None
) -> NDArray[ScalarT]: ...
@overload
...
def as_ctypes(
    obj: NDArray[np.void],
) -> ct.Array[Any]: ...  # `ct.Union` or `ct.Structure`

...
from numpy._typing import (ArrayLike, NDArray, _16Bit, _32Bit, _64Bit,
                           _AnyShape, _ArrayLike, _NestedSequence,
                           _ScalarLike_co, _Shape, _SupportsArray)
...
type _NumberMax64 = np.number[_64Bit] | np.number[_32Bit] | np.number[
    _16Bit
] | np.integer
...
def real_if_close[RealT: _ToReal](
    a: _ArrayLike[RealT], tol: float = 100
) -> NDArray[RealT]: ...
@overload
def real_if_close(
    a: _ArrayLike[np.complex64], tol: float = 100
) -> NDArray[np.float32 | np.complex64]: ...
@overload
def real_if_close(

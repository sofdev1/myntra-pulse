            ...
            if "*" in var["kindselector"]:
                ...
                try:
                    ...
                    try:
                        ...
                        errmess(
                            f'getctype: "{typespec}({kind=})" is mapped to C '
...
def getstrlength(var):
    ...
    if re.match(r"\(\s*(\*|:)\s*\)", len) or re.match(r"(\*|:)", len):
        if isintent_hide(var):
            errmess(
                f"getstrlength:intent(hide): expected a string with defined length "
                f"but got: {var!r}\n"
            )
        len = "-1"
    return len
    ...
    if isstring(var) and not isarray(var):
        ...
        for d in var["dimension"]:
            ...
            if d not in ["*", ":", "(*)", "(:)"]:
                ...
                outmess(
                    "getarrdims:warning: assumed shape array, using 0 "
                    f"instead of {d!r}\n"
                )
...
def getpydocsign(a, var):
    ...
    if isscalar(var):
        if isintent_inout(var):
            sig = (
                f"{a} : {opt} rank-0 array({c2py_map[ctype]},"
        ...
        if isintent_inout(var):
            sig = (
                f"{a} : {opt} rank-0 array(string(len={getstrlength(var)}),"
                f"'c'){init}"
            )
        else:
        ...
        dim_str = ",".join(dim)
        sig = (
            f"{a} : {opt} rank-{rank} array('{c2pycode_map[ctype]}') with "
            f"bounds ({dim_str}){init}"
        )
        if a == out_a:
            sigout = (
                f"{a} : rank-{rank} array('{c2pycode_map[ctype]}') with "
                f"bounds ({dim_str})"
            )
        else:
            sigout = (
                f"{out_a} : rank-{rank} array('{c2pycode_map[ctype]}') with "
...
def sign2map(a, var):
    ...
    if isexternal(var):
        ...
        if a in lcb_map:
            ...
            errmess(
                f"sign2map: Confused: external {a} is not in "
    ...
    if debugcapi(var):
        ...
        if isexternal(var):
            ...
            ret["vardebuginfo"] = (
                f"debug-capi:{ret['ctype']} " f"{a}={ret['showinit']}:{rl_str}"
            )
        if isscalar(var):
...
def routsign2map(rout):
    ...
    if "use" in rout:
        ...
        errmess(
            f"routsign2map: Confused: function {ret['name']} has externals "
    ...
    if isfunction(rout):
        ...
        if debugcapi(rout):
            if ret["ctype"] in cformat_map:
                ret["routdebugshowvalue"] = (
                    "debug-capi:" f"{a}={cformat_map[ret['ctype']]}"
                )
        ...
        if isstringfunction(rout):
            ...
            if ret["rlength"] == "-1":
                errmess(
                    "routsign2map: expected explicit specification of the length "

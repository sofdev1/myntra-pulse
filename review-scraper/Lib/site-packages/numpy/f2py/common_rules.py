...
def buildhooks(m):
    ...
    for name, vnames, vars in findcommonblocks(m):
        ...
        if hnames:
            outmess(
                f'\t\tConstructing COMMON block support for "{name}"...\n\t\t  '
                f"{inames_str}\n\t\t  Hidden: {hnames_str}\n"
            )
        else:
            outmess(
        ...
        cadd(
            f"extern void {F_FUNC}(f2pyinit{lower_name},F2PYINIT{name.upper()})"
            f"(void(*)({arg_types_str}));"
        )
        cadd(f"static void f2py_init_{name}(void) {{")
        cadd(
            f"\t{F_FUNC}(f2pyinit{lower_name},F2PYINIT{name.upper()})"
            f"(f2py_setup_{name});"
        )
        cadd("}\n")

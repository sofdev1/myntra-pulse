...
def readfortrancode(ffile, dowithline=show, istop=1):
    ...
    while True:
        ...
        if fin.isfirstline():
            ...
            outmess(
                f"\tReading file {currentfilename!r} "
        ...
        filepositiontext = (
            f"Line #{fin.filelineno() - 1} " f'in {currentfilename}:"{l1}"\n\t'
        )
        ...
        if m:
            ...
            if os.path.isfile(fn):
                ...
                if not foundfile:
                    outmess(
                        f"readfortrancode: could not find include file {fn!r} "
...
def crackline(line, reset=0):
    ...
    if reset > 0:
        ...
        while groupcounter > fl:
            outmess(
                f"crackline: groupcounter={groupcounter!r} groupname={groupname!r}\n"
            )
            outmess(
    ...
    if not m:
        ...
        if groupcounter <= 0:
            raise Exception(
                f"crackline: groupcounter(={groupcounter}) is nonpositive. "
                "Check the blocks."
            )
        m1 = beginpattern[0].match(line)
        if (m1) and (not m1.group("this") == groupname[groupcounter]):
            raise Exception(
                f'crackline: End group {m1.group("this")!r} '
...
def analyzeline(m, case, line):
    ...
    if case in ["begin", "call", "callfun"]:
        ...
        if needinterface:
            ...
            groupcache[groupcounter][
                "from"
            ] = f"{prev_group['from']}:{prev_group['name']}"
            groupcache[groupcounter]["body"] = []
            groupcache[groupcounter]["externals"] = []
            groupcache[groupcounter]["interfaced"] = []
            groupcache[groupcounter]["vars"] = {}
        ...
        if groupcounter == 1:
            ...
            groupcache[groupcounter][
                "from"
            ] = f"{groupcache[groupcounter - 1]['from']}:{currentfilename}"
        else:
            prev_group = groupcache[groupcounter - 1]
            groupcache[groupcounter][
                "from"
        ...
        if i < 0 and case == "intent":
            ...
            if ll[i:] == "::" and "args" in groupcache[groupcounter]:
                outmess(
                    "All arguments will have attribute " f'{m.group("this")}{ll[:i]}\n'
                )
        ...
        if len(ch) > 1:
            pl = ch[0]
            outmess(
                "analyzeline: cannot handle multiple attributes without "
        ...
        for e in [x.strip() for x in markoutercomma(ll).split("@,@")]:
            ...
            if not m1:
                if case in ["public", "private"]:
                    ...
                    print(m.groupdict())
                    outmess(
                        f"analyzeline: no name pattern found in {case} statement "
                        f"for {e!r}. Skipping.\n"
                    )
                    continue
            else:
                    ...
                    if k not in groupcache[groupcounter]["args"]:
                        if groupcounter > 1:
                            ...
                            if k != groupcache[groupcounter]["name"]:
                                outmess(
                                    f"analyzeline: appending intent(callback) {k}"
                        ...
                        errmess(
                            f"analyzeline: intent(callback) {k} is already"
                            " in argument list\n"
                        )
            if case in [
                "optional",
        ...
        for e in markoutercomma(ll).split("@,@"):
            ...
            if "=" in edecl[k] and (not edecl[k]["="] == initexpr):
                outmess(
                    f'analyzeline: Overwriting the value of parameter "{k}" '
            ...
            try:
                ...
                errmess(
                    f"analyzeline: Failed to evaluate {initexpr!r}. "
                    f"Ignoring: {msg}\n"
                )
                continue
            edecl[k]["="] = repr(v)
    ...
    for e in el1:
        ...
        if ename in groupcache[groupcounter]["vars"]:
            ...
            if not_has_typespec:
                ...
                outmess(
                    f'updatevars: attempt to change the type of "{ename}" '
            ...
            if "kindselector" not in edecl:
                ...
                for k in list(kindselect.keys()):
                    if k in edecl["kindselector"] and (
                        ...
                        outmess(
                            "updatevars: attempt to change the kindselector "
            ...
            if "charselector" not in edecl and charselect:
                if not_has_typespec:
                    ...
                    errmess(
                        f"updatevars:{ename}: attempt to change empty charselector "
                ...
                for k in list(charselect.keys()):
                    if k in edecl["charselector"] and (
                        ...
                        outmess(
                            f'updatevars: attempt to change the charselector "{k}" '
            ...
            if "typename" not in edecl:
                ...
                outmess(
                    f'updatevars: attempt to change the typename of "{ename}" '
            ...
            if m1:
                ...
                if "len" in d1 and "array" in d1:
                    if d1["len"] == "":
                        ...
                        array_spec = d1["array"]
                        errmess(
                            f'updatevars: "{typespec} {e}" is mapped to '
                ...
                if "init" in d1:
                    if "=" in edecl and (not edecl["="] == d1["init"]):
                        ...
                        new_init = d1["init"]
                        outmess(
                            "updatevars: attempt to change the init expression of "
                    ...
                    if "attrspec" not in edecl or (not edecl["attrspec"]):
                        ...
                        for dm1 in edecl["attrspec"]:
                            if dm1[:9] == "dimension" and dm1 != dm:
                                ...
                                errmess(
                                    f"updatevars:{ename}: attempt to change "
...
def get_useparameters(block, param_map=None):
    ...
    for usename, mapping in list(usedict.items()):
        ...
        if usename not in f90modulevars:
            outmess(
                f"get_useparameters: no module {usename} info used by "
                f'{block.get("name")}\n'
            )
            continue
        ...
        for k, v in list(params.items()):
            if k in param_map:
                outmess(
                    f"get_useparameters: overriding parameter {k!r} with"
...
def _eval_scalar(value, params):
    ...
    try:
        ...
        errmess(
            f'"{msg}" in evaluating {value!r} ' f"(available names: {list(params)})\n"
        )
    return value
        ...
        if "typespec" not in vars[n]:
            if not ("attrspec" in vars[n] and "external" in vars[n]["attrspec"]):
                if implicitrules:
                    ...
                    outmess(
                        f"analyzevars: typespec of variable {n!r} is not defined "
...
def crack2fortrangen(block, tab="\n", as_interface=False):
    ...
    if "f2pyenhancements" in block:
        for k in list(block["f2pyenhancements"].keys()):
            f2pyenhancements = (
                f"{f2pyenhancements}{tab + tabchar}{k} "
    ...
    if intent_lst:
        f2pyenhancements = (
            f"{f2pyenhancements}{tab + tabchar}"
    ...
    if blocktype == "block data" and name == "_BLOCK_DATA_":
        name = ""
    ret = (
        f"{tab}{prefix}{blocktype} {name}{args}{result} "
